...
class DiffusionModel3D(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        if self.hparams.backbone.lower() == "unet3d_pixelshuffle_inject":
            from models.module_UNet3D_pixelshuffle_inject import (
                module_UNet3D_pixelshuffle_inject as backbone,
            )
        else:
            raise ValueError(
                "Unknown backbone architecture {0}!".format(self.hparams.backbone)
            )
        ...
        self.DiffusionTrainer = GaussianDiffusionTrainer(
            self.hparams.num_timesteps, schedule=self.hparams.diffusion_schedule
        ).to(device)
        self.DiffusionSampler = GaussianDiffusionSampler(
            self.network,
            self.hparams.num_timesteps,
            t_start=self.hparams.num_timesteps,
            t_save=self.hparams.num_timesteps,
            t_step=1,
    ...
    def forward(self, z, t, phases):
        return self.network(z, t, phases)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = F.l1_loss(x[:, 0:1, ...] - x_hat, x[:, 0:1, ...])
    ...
    def train_dataloader(self):
        if self.hparams.train_list is None:
            ...
            dataset = MeristemH5Dataset(
                ...
                data_norm=self.hparams.data_norm,
                no_mask=True,
                boundary_handling="none",
                image_noise_channel=self.hparams.image_noise_channel,
                mask_noise_channel=self.hparams.mask_noise_channel,
    ...
    def test_dataloader(self):
        if self.hparams.test_list is None:
            ...
            return DataLoader(
                dataset, batch_size=self.hparams.batch_size, num_workers=36
            )
    ...
    def on_train_epoch_end(self):
        ...
        with torch.no_grad():
            ...
            # get x_0
            x_0, _ = self.DiffusionSampler(input_patch[:, 0:1, ...], self.phases)

            # log sampled images
            prediction_grid = torchvision.utils.make_grid(
                x_0[..., int(self.hparams.patch_size[0] // 2), :, :]
            )
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            "--backbone",
            default="UNet3D_PixelShuffle_inject",
            type=str,
            help="which model to load (UNet3D_PixelShuffle_inject)",
        )
        parser.add_argument("--in_channels", default=1, type=int)
        parser.add_argument("--out_channels", default=1, type=int)
        parser.add_argument("--feat_channels", default=16, type=int)
        parser.add_argument("--t_channels", default=16, type=int)
        parser.add_argument("--patch_size", default=(96, 96, 96), type=int, nargs="+")
        ...
        parser.add_argument(
            "--mask_groups", default=("data/diffusion_mask",), type=str, nargs="+"
        )
